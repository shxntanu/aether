...
func (s *Service) CompleteLogin(ctx context.Context, identity Identity) (string, domain.Member, error) {
	if !identity.EmailVerified {
		return "", domain.Member{}, ErrEmailUnverified
	}
	member, err := s.repository.GetMemberByEmail(ctx, normalizeEmail(identity.Email))
	if errors.Is(err, domain.ErrNotFound) {
		return "", domain.Member{}, ErrNotAllowlisted
	}
	if err != nil {
		return "", domain.Member{}, err
	}
	...
	token := s.newSecret()
	now := s.now()
	session := domain.Session{ID: s.newSecret(), TokenHash: tokenHash(token), MemberID: member.ID, CreatedAt: now, ExpiresAt: now.Add(7 * 24 * time.Hour)}
